local aes = require("meshtastic.aeslua.aes");
local util = require("meshtastic.aeslua.util");
local buffer = require("meshtastic.aeslua.buffer");
...
-- Counter mode decrypt function
function public.decryptCTR(keySched, byteData, iv)
    local nextIV = {};
    for j = 1,16 do
        nextIV[j] = iv[j];
    end
    nextIV[#nextIV] = nextIV[#nextIV] + 1;
