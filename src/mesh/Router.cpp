...
#define MAX_RX_FROMRADIO                                                                                                         \
    10 // max number of packets destined to our queue, we dispatch packets quickly so it doesn't need to be big
...
meshtastic_Routing_Error perhapsEncode(meshtastic_MeshPacket *p)
{
    ...
    if (p->which_payload_variant == meshtastic_MeshPacket_decoded_tag) {
        ...
        if (isFromUs(p) &&
            ...
            !(p->pki_encrypted != true && p->channel > 0) &&  // I.e., && (p->pki_encrypted || p->channel == 0)
